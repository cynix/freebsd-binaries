...
func (cp *ContainerProject) BuildContainer(gh *github.Client, version, name string) error {
	if name != cp.Name {
		return fmt.Errorf("no such container to build: %q", name)
	}
...
func (conf ContainerConfig) Build(gh *github.Client, ci containerInfo, archs []string) (err error) {
	base := conf.base()
	if err = utils.Command("podman", "pull", base).Run(); err != nil {
		return
	}

	if ci.FreeBSD, err = utils.Command("podman", "image", "inspect", "--format={{index .Annotations \"org.freebsd.version\"}}", base).First(); err != nil {
		return
	}
	...
	for _, ci.Arch = range archs {
		if tagged, err = conf.build(gh, ci, latest, tagged, base); err != nil {
			return
		}
	}

	if err = utils.Command("buildah", "manifest", "push", "--all", latest, "docker://"+latest).Run(); err != nil {
		return
	}

	if tagged != "" {
		if err = utils.Command("buildah", "manifest", "push", "--all", latest, "docker://"+tagged).Run(); err != nil {
			return
		}
	}
...
func (conf ContainerConfig) build(gh *github.Client, ci containerInfo, latest, tagged, base string) (string, error) {
	core.Infof("Building arch: %s", ci.Arch)

	c := &container{manifest: latest}
	...
	if fi, err := os.Stat(path.Join(ci.Package, "root")); err == nil && fi.IsDir() {
		...
		if err = utils.CopyDir(c.root, path.Join(ci.Package, "root")); err != nil {
			return tagged, err
		}
	...
	if user, uid, _ := strings.Cut(conf.User, "="); uid != "" {
		...
		if err := utils.Command("pw", "-R", c.root, "groupadd", "-n", user, "-g", uid).Run(); err != nil {
			return tagged, err
		}
		if err := utils.Command("pw", "-R", c.root, "useradd", "-n", user, "-u", uid, "-g", user, "-d", "/nonexistent", "-s", "/sbin/nologin").Run(); err != nil {
			return tagged, err
		}
	}
	...
	for _, a := range conf.Assets {
		...
		if len(conf.Entrypoint) == 0 && ai.InferredEntrypoint != "" {
			...
			if _, ok := a.Deployable.(FileAsset); ok {
				if err := os.Chmod(path.Join(c.root, ai.InferredEntrypoint), 0o755); err != nil {
					return tagged, err
				}
			}
		}
	...
	if err := os.Chmod(path.Join(c.root, "/usr/local/sbin"), 0o711); err != nil && err != os.ErrNotExist {
		return tagged, err
	}
	...
	if conf.Script != "" {
		...
		if core.Group("Running build script", func() {
			err = utils.Command("sh", "-ex").In(c.root).WithInput(conf.Script).Run()
		}); err != nil {
			return tagged, err
		}
	}
...
func (c *container) Create(base, arch string) (err error) {
	...
	if c.id, err = utils.Command("buildah", "from", "--arch="+arch, "ghcr.io/cynix/"+base).First(); err != nil {
		return
	}
...
func (c *container) Buildah(command string, args ...string) *utils.Cmd {
	return utils.Command("buildah", slices.Concat([]string{command}, args, []string{c.id})...)
}
